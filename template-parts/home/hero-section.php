<?php
  global $drdev_assets;
  if (!is_array($drdev_assets)) {
    $drdev_assets = drdev_get_assets_data(); 
  }
?>
<section aria-label="Hero principal con promociones" class="relative w-full h-[85vh]">
  <!-- Slider Container -->
  <div class="swiper hero-slider h-full">
    <div class="swiper-wrapper" aria-live="polite">
      <!-- Slide 1 (Imagen original) -->
      <div class="swiper-slide" role="group" aria-label="Slide 1 de 2">
        <div class="relative w-full h-full bg-cover bg-center hero-bg">
          <div class="grid grid-cols-1 md:grid-cols-5 h-full items-start md:items-center">
            <div class="md:col-span-2 order-2 md:order-1">
            </div>
            <div class="relative z-10 md:col-span-3 order-1 md:order-2 flex flex-col items-center justify-center text-primary text-center font-montserrat px-4 py-8 md:px-12">
              <h1 class="text-[26px] md:text-[60px] 2xl:text-[70px] leading-[1.1em] font-black uppercase drop-shadow-[0_0_2px_rgba(0,0,0,0.25)]">
                ¡Envía paquetes<br />a Cuba de manera<br />fácil y segura!
              </h1>
              <h2 class="mt-4 sm:mt-6 text-[12px] md:text-[32px] leading-[1.2em] font-normal drop-shadow-[0_0_2px_rgba(0,0,0,0.25)]">
                Con opciones de envío aéreo y marítimo,<br />siempre en su puerta.
              </h2>
              <a href="#formulario-envio" class="inline-block mt-4 sm:mt-8 px-20 py-4 bg-primary text-white font-bold text-base leading-[1.2em] rounded">
                Envía ahora
              </a>
            </div>
          </div>
        </div>
      </div>

      <!-- Slide 2  -->
      <div class="swiper-slide" role="group" aria-label="Slide 2 de 2">
        <div class="relative w-full h-full bg-cover bg-center hero-bg-one">
          <div class="grid grid-cols-1 md:grid-cols-5 h-full items-start md:items-center">
            <!-- Columna izquierda vacía -->
            <div class="md:col-span-3 order-2 md:order-1"></div>
            
            <!-- Columna derecha con texto -->
            <div class="relative z-10 md:col-span-2 order-1 md:order-2 flex flex-col items-center justify-center text-center font-montserrat px-4 py-8 md:px-0">
              <h2 class="text-white text-[24px] sm:text-[36px] md:text-[48px] 2xl:text-[68px] leading-[1.2em] font-black uppercase drop-shadow-[0_0_2px_rgba(0,0,0,0.25)]">
                OFERTA Recarga Internacional Cubacel<br />Datos ilimitados desde 500 CUP
              </h2>
              <h3 class="text-secondary mt-6 text-[14px] sm:text-[32px] leading-[1.2em] font-normal drop-shadow-[0_0_2px_rgba(0,0,0,0.25)]">
                Hasta (fecha de culminación)
              </h3>
              <a href="https://wa.me/<?php echo esc_attr($drdev_assets['whatsapp_clean']); ?>" class="inline-block mt-8 px-6 py-3 bg-secondary text-white font-bold text-base leading-[1.2em] rounded">
                Contacta para Recargar Ahora
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>

    <!-- Paginación (puntos) -->
    <div class="swiper-pagination !bottom-8"></div>
  </div>
</section>